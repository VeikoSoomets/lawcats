...
var _servicesMessagingService = require('../services/MessagingService');

var _servicesMessagingService2 = _interopRequireDefault(_servicesMessagingService);

var _controllersSettingsController = require('../controllers/SettingsController');

var _controllersSettingsController2 = _interopRequireDefault(_controllersSettingsController);

var _directivesNgCategories = require('../directives/ngCategories');

var _directivesNgCategories2 = _interopRequireDefault(_directivesNgCategories);

angular.module('settingsApp', []).service('MessagingService', _servicesMessagingService2['default']).directive('ngCategories', _directivesNgCategories2['default']).controller('SettingsController', _controllersSettingsController2['default']);
/**
...
},{"../controllers/SettingsController":4,"../directives/ngCategories":5,"../services/MessagingService":6}],4:[function(require,module,exports){
/**
...
var SettingsController = (function () {
  function SettingsController(MessagingService, $http) {
  ...
  _createClass(SettingsController, [{
    ...
    key: 'pullGroupData',
    value: function pullGroupData() {
      var _this2 = this;

      this.$http.get('/app/settings/data').success(function (response) {
        _this2.groupMembers = response.data.group_members;
        _this2.userEmail = response.data.group_to_answer;
        _this2.groupMaster = response.data.group_master;
      }).error(function (err) {
        return _this2.MessagingService.danger(err);
      });
    }
  }, {
    key: 'addToUserGroup',
    value: function addToUserGroup() {
      if (this.userGroup['new'].trim()) {
        ...
        var err = 'Please add an email to new usergroup member.';
    ...
    key: 'apiRequest',
    value: function apiRequest(action, data) {
      var _this3 = this;

      var url = arguments.length <= 2 || arguments[2] === undefined ? '/app/settings' : arguments[2];
      ...
      this.$http({
        ...
        if (data.type === 'danger') {
          _this3.MessagingService.danger(data.message);
        } else if (data.type === 'success') {
          _this3.MessagingService.success(data.message);
        }
      }).error(function (err) {
        _this3.MessagingService.danger(err);
      });
    }
  }, {
    key: 'changePassword',
    value: function changePassword() {
      var firstPass = this.passwordChange.newPassword;
    ...
    value: function addNewSource() {
      if (this.newSource.link.trim() && this.newSource.description.trim()) {
        ...
        var msg = 'Please add a link and description to your new source.';
        this.MessagingService.danger(msg);
      }
    }
  }, {
    key: 'leaveUserGroup',
    value: function leaveUserGroup() {
    ...
    key: 'removeFromUserGroup',
    value: function removeFromUserGroup(user) {
    ...
    key: 'createUserGroup',
    value: function createUserGroup() {
      if (this.userGroup.name.trim()) {
...
var MessagingService = (function () {
  ...
  _createClass(MessagingService, [{
    key: 'info',
    value: function info(message, time) {
      var timeOut = time || this.messageTime;
      _toastr2['default'].info(message, null, { timeOut: timeOut });
    }
    ...
    key: 'success',
    value: function success(message, time) {
      var timeOut = time || this.messageTime;
      _toastr2['default'].success(message, null, { timeOut: timeOut });
    }
    ...
    key: 'danger',
    value: function danger(message, time) {
      var timeOut = time || this.messageTime;
      _toastr2['default'].error(message, null, { timeOut: timeOut });
    }
  }]);
